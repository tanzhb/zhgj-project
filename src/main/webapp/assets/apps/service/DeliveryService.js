...
angular.module('MetronicApp').factory('DeliveryService', ['$rootScope', '$http', '$q','$filter', function($rootScope, $http, $q,$filter){
    ...
    var factory = {
    		...
    		saveDeliverFile:saveDeliverFile,
    		saveFile:saveFile,
    ...
    //保存附件
    function saveDeliverFile(File){
		var deferred = $q.defer();
		var params = {};
		params = JSON.stringify(File);
		$http.post("rest/delivery/saveDeliverFile", params
 ...
 function saveAllDeliveryMateriel (deliveryMateriel){
    ...
    function saveBasicInfo($scope,type){
		...
		$.post("rest/delivery/saveBasicInfo", {
			...
			takeDeliverer:$scope.deliver.takeDeliverer,
			takeContactNum:$scope.delivery.takeContactNum,
			takeTransportRemark:$scope.deliver.takeTransportRemark,
  ...
  //通过企业id找联系地址集合
    ...
    //判断订单物料是否均无库存
    function judgeIsStock(serialNum){
    	var deferred = $q.defer();  

        $http.get("rest/delivery/judgeIsStock", {params:{serialNum:serialNum}})
        .success(function (data) {  
            // 如果连接成功，延时返回给调用者  
            deferred.resolve(data);  
        }).error(function () {  
