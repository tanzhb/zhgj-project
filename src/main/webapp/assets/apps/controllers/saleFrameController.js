...
angular.module('MetronicApp').controller('saleFrameController', ['$rootScope', '$scope', 'settings','orderService','$filter',
    ...
    $scope.$on('$viewContentLoaded', function() {   
        ...
        if($state.current.name=="saleFrame"){
...
/*    $scope.repeatDone = function(scope){
   ...
   $scope.datepickerInit = function(scope){
    ...
    var table;
    var tableAjaxUrl = "rest/order/findFrameList?type=sale";
		        						...
		        						},
		        						{
		        							...
		        							mRender : function(data,type,row,meta) {
		        								if(isNull(row.version)&&isNull(row.deleteReason)){
		        									return "<a href='javascript:void(0);' onclick=\"userCancelApply('"+row.taskId+"','"+row.processInstanceId+"','endTaskTable','saleFram')\">取消申请</a>";
