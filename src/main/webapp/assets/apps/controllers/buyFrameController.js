...
//            	setTimeout($scope.autoSave, 300000);//5分钟框架协议自动保存一次
    ...
    function doFrame(_url, mydata, modal){
    	$.ajax( {
	        url : _url,
	        dataType:"text",
	        type: 'POST',
	...
	$scope.replyFrame = function() {
	    var mydata={"processInstanceId":$("#processInstanceId").val(),
				"reApply":true,"frameId":$scope.buyFrame.id,"reason":$scope.buyFrame.remark,"frameType":'buyFrame'};
	...
	$scope.cancelApply = function() {
	     var mydata={"processInstanceId":$("#processInstanceId").val(),
				"reApply":false,"frameId":$scope.buyFrame.id,"reason":$scope.buyFrame.remark,"frameType":'buyFrame'};
		var _url = ctx + "rest/order/modifyFrame/" + $("#taskId").val();
		doFrame(_url, mydata, 'modify' );
	};
		        						...
		        						},
		        						{
		        							...
		        							mRender : function(data,type,row,meta) {
		        								if(isNull(row.version)&&isNull(row.deleteReason)){
		        									return "<a href='javascript:void(0);' onclick=\"userCancelApply('"+row.taskId+"','"+row.processInstanceId+"','endTaskTable','buyFram')\">取消申请</a>";
...
//	 	        };
	 	  	...
	 	  	$scope.showSX=function(judgeString){//增加交付条款地址筛选 
	 			debugger;
	 			if(judgeString=='f'){
	 				if($scope.showSXf!='1'){
	 					$scope.showSXf='1';
