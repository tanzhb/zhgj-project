...
//        	loadMainFramTable();// 框架订单列表
        	...
        	if($stateParams.tabHref=='2'&&!isNull($stateParams.serialNum)){//点击消息查看发货计划
        		$scope.jumpToGetDeliveryInfo($stateParams.serialNum);
        	}
        	}else{
   ...
   $scope.datepickerInit = function(scope){
    ...
    $scope.autoSave  = function() {
		...
		if(isExist){
			...
			if($state.current.name=="addSupplyOrder"&&$scope.buyOrderInput != true&&$('#form_sample_1').valid()){//处于编辑状态且验证通过
	    		if($scope.buyOrder.orderDate=='') {// 日期为空的处理
	    			$scope.buyOrder.orderDate=null;
	    		}
	    		orderService.save($scope.buyOrder).then(
	...
	if($state.current.name=="addSupplyOrder"){
...
//                 serverSide: true,
    ...
    $scope.editOrder  = function(serialNum) {// 进入编辑页面
    	...
    	if(isNull(serialNum)){
    		$state.go('addSupplyOrder',{serialNum:$scope.buyOrder.serialNum})
    	}else{
    		$state.go('addSupplyOrder',{serialNum:serialNum})
    	}
    	
    };
    $scope.deliveryAdd= function(serialNum) {
   	 
    	$state.go('addDelivery',{oprateType:"forSupplyOrder",orderSerialNum:serialNum});
      }
    $scope.pingTaiSubmit  = function(serialNum,orderAmount) {// 供应商/采购商提交给平台
...
// serverSide: true,
               ...
               'aoColumnDefs' : [ {
					...
					'render' : function(data,
							type, row, meta) {
						var clickhtm = '<a href="javascript:void(0);" ng-click="viewSupplyOrder(\''+row.serialNum+'\')">'+data+'</a></br>'
						if(row.status==0){
...
//							return clickhtm + '<span ng-click="viewOrderLog(\''+row.serialNum+'\')"  style="color:#fcb95b">待审批</span>';//待签合同
					...
					'render' : function(data,
						...
						if(isNull(row.payAmount)||row.payAmount==0){
							htm = (isNull(data)?'<span style="color:#FCB95B">0</span>':'<span style="color:#FCB95B">'+data+'</span>'+'</br>')+'（已收 '+'<span style="color:#FCB95B">0</span>'+'）'+'</br>'
						}else{
...
//	          		    	$location.search({serialNum:data.serialNum,view:'all'});
	...
	/*		       	scope._CSD.billingAmount = (Number($scope._totaldeliveryAmount()) - Number($scope._totalbillingAmount()) + Number(scope._CSD.billingAmount)).toFixed(2);
		     ...
		     /***************日志表格 start************************/
		 	var logTable;
		 	var  outRecordTable,tableUrl,type,tableId;// 
			 ...
			 function showPayLogTable(url){
		 	 ...
		 	 var aoColumns,tableId;
		 	 function showLogTable(url){
		 		 if($scope.showMoneyData){//显示金额和日期
		 			aoColumns =[
								{
										mData : 'operationDesc'
									},
		 			...
		 			tableId="select_payOperateLog";
		 		 }else if($scope.showNumberData){//显示数量和日期
		 			aoColumns=[
								{
										mData : 'operationDesc',
	        							mRender : function(
		 			...
		 			tableId="select_deliverOperateLog";
		 		 }else{
		 			aoColumns= [
						{
							mData : 'operationDesc'
